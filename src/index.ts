...
try{
  ...
  if(!chat_id) return res.status(404).json("Chatid is not valid for this Video")
    ...
    const msg = message[0]
    if(!msg?.media) return res.status(400).send("Video Not Found")
...
app.get('stream/:message_id',async(req,res)=>{
const {message_id} = req.params
})
